...
byint<-btb %>% group_by(interviewee.GRname,interviewee.id, interviewee.gender.use,interviewee.birthdate.use,Date) %>% summarise(countF=rsum(author.gender.use=="f"),countM=rsum(author.gender.use=="m"),countauth=countN(author.gender.use=="f")) %>% mutate(percF=countF/countauth,age=as.period(interval(interviewee.birthdate.use,Sys.Date()))$year) %>% mutate(varf=(1-percF)/countauth) %>% arrange(percF)
...
Older interviewees are both more likely to be male (`r 100*rmeanr(byint %>% filter(interviewee.gender.use=="m") %>%  select("interviewee.birthdate.use") %>% transmute(old=year(interviewee.birthdate.use)<1960) %>% pull())`% of male interviewees were born before 1960, vs `r 100*rmeanr(byint %>% filter(interviewee.gender.use=="f") %>%  select("interviewee.birthdate.use") %>% transmute(old=year(interviewee.birthdate.use)<1960) %>% pull())`% of female interviewees) and more likely to have recommended a lower percentage of female authors.

```{r}
ggplot(byint,aes(x=interviewee.birthdate.use,y = percF,color=interviewee.gender.use,size=countauth)) +geom_point() +scale_color_manual(values=plotcolors$color, name="interviewee\ngender") + scale_size(name="authors\nmentioned") + ylab("Female authors mentioned (percent)") + xlab("Interviewee birthdate") +scale_y_continuous(labels = function(x){as.character(100*x)})
```
...
ggplot(heatmapintplot, aes(x = interviewee.x,y = interviewee.y)) + 
  ...
  scale_fill_brewer(palette = "Spectral",type = seq,direction = -1,guide = guide_legend(title = NULL)) +
  coord_fixed() +
