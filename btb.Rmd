...
ggplot(data =caldata ) + 
  ...
  scale_y_continuous(breaks = caldata$rnk,labels=rev(paste(caldata$monthlabel,as.character(caldata$d))),name = element_blank(),expand = c(0,0)) + guides(fill=F) + 
...
Older interviewees are both more likely to be male (**`r 100*rmeanr(byint %>% filter(interviewee.gender.use=="m") %>%  select("interviewee.birthdate") %>% transmute(old=year(interviewee.birthdate)<1960) %>% pull())`%** of male interviewees were born before 1960, vs **`r 100*rmeanr(byint %>% filter(interviewee.gender.use=="f") %>%  select("interviewee.birthdate") %>% transmute(old=year(interviewee.birthdate)<1960) %>% pull())`%** of female interviewees) and slightly more likely to have recommended a lower percentage of female authors. 
